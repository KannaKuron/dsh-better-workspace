...
11. **右键即操作**:全行 onContextMenu 打开自绘菜单(fixed overlay,坐标来自事件),原 ⋯ 按钮已移除、不要加回;菜单动作(rename/delete/fork/archive/rename-folder/rename-sgroup/customize)。**v0.12.0 起分组菜单只剩重命名分组 + 自定义外观**(显式空分组与 new-subfolder/new-subworkspace 随数据通路退役;分组靠改名/拖拽派生,不靠空壳容器)。**v0.15.0 起四类行各有「移动到分组…」(`menu.moveToGroup`),但按开关逐层门控:工作区行 / 工作区分组行只在 workspaceSlash 开启时出现,会话行 / 会话分组行只看 sessionSlash**——容器不存在时不得承诺它;同一个 dialog 还服务于区头「新建分组」按钮(picker 形态,候选按开启的层收窄)与它的拖拽投放区(未开启那一层的行拖上去不高亮、不接管)。
