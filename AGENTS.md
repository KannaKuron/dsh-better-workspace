...
6. **持久化**两个 key:**dsh.betterWorkspace.view.v1** 走 dsh 客户端 store(defineStore + persist;显式空分组 folders、折叠状态 expanded/sessionsExpanded/sessionGroups、prefs、styling);**dsh.betterWorkspace.titles.v1** 自 **0.9.6** 起是**普通 localStorage 缓存**(模块级 titleCache 对象 + loadTitleCache 整值恢复 + scheduleTitleSave 200ms 防抖整写,{ byId: { id: { title, at } } })——issue #1 证明它绝不能做成响应式 store。不要为动态实验另起第三种持久化。
