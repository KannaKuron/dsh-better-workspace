...
12. **外观自定义**:store.styling 键约定 folder:/workspace:/session:/sgroup: <id|path>;值 { color, glow(0/4/8/14), icon }。icon 兼容矩阵:legacy 三值 solid/outline/none(实心/空心文件夹/无)或任意 primitives 图标名(ICON_CHOICES 网格,25 个同库图标)。**发光只有文字**:textShadow 只作用于行内文字,禁止行外圈 boxShadow(用户明确不要外框发光)。default 提交 = null 清除;删除条目 = setStyling(key, null)。
