...
**类型**:feat(随机配色做到**正文级 AA**:双主题槽位色池 + 槽位 id 存储)

用户决策:**选 B** —— 随机配色的可读性要真正达到 WCAG **正文级 AA(对比度 4.5)**,而不是停在「两底 ≥3.0」。方案选择:**按主题两套色池 + 存槽位 id(`auto:3`)而非色值**(详见下)。

### 为什么是「槽位 id」而不是「分配时按当时主题挑色」

- 行文字色是固定 hex 存储,换主题后必然失配。**方案 ii**(分配时按当时主题选池)简单,但换主题后既有自动色全部错配;要缓解就得在主题切换时对「自动分配且未被手动改过」的项重映射 —— 而**「是否被手动改过」在纯 hex 存储下不可判定**(用户手动挑的颜色完全可能等于池内色),只能在存储里再加标记,最终还是要动数据形状。
- **方案 i(采用)**:存储 `color: 'auto:<index>'`,渲染时按**当前主题**解析成 `{light|dark}` 中的一个色。收益:换主题自动换色、**零迁移**、跨端同步天然正确(宿主副本存 token,各端各自上自己的主题色)、洗牌袋仍按槽位抽取(主题切换不会让「最近抽过」失效)。代价:多一层解析(一个正则 + 一次查表),以及**槽位表必须 append-only**(已存 token 要永远解析得出;越界索引夹到槽位 0,绝不渲染成空色)。
- 兼容性:①**旧 hex 数据照旧可用**(`resolveColorValue` 对非 token 原样透传,老的手动色/上一次自动分配出的 hex 都继续渲染);②**手动设置的颜色仍存 hex、永不被覆盖**(存在性门控只看键在不在,不看值);③与 `stylingMap` / `setStylingMany` **零改动**(token 只是字符串);④`colorToRgb()` 只认 6 位 hex ⇒ token 不会误入描边与取色器,相关调用点(`rowStyleOf` / `strokeColorOf` 的 auto 极色 / `AppearanceControls` 的展示值与预览)都已先解析。
- **不做自动迁移**:v0.24.0 从未发布(npm 上没有该版本),唯一的 hex 自动色存在于开发期 tarball;而「迁移旧 hex 到槽位」在纯 hex 下无法区分自动/手动,强行迁移会违背「绝不覆盖手动设置」。老 hex 保持原样渲染即可。

### 双主题色池(真实计算,非估算)

色值由脚本按「同色相槽位、饱和度优先、亮度贴近 5.2」搜索得到。**背景取自真机实测**(见下方真机段):浅色 `--dsw-alias-bg-base` = `#ffffff`、悬停/当前行叠加约 `#f7f8fa` ⇒ 判据取**更暗的 `#ebedf0`**(保守);深色 base `#151517`、layer-1 `#232324`、layer-2 `#2c2c2e` ⇒ 判据取**最亮的 `#2c2c2e`**(保守)。下表「对自主题」列为对上述**保守面**的实测对比度。

| 槽位 | 色相 | 浅色主题色 | 对 #ebedf0 | 对白 | 深色主题色 | 对 #2c2c2e | 对 #151517 |
|---|---|---|---|---|---|---|---|
| red | 4° | `#c40d00` | 5.26 | 6.17 | `#ff7369` | 5.25 | 6.87 |
| amber | 32° | `#965000` | 5.20 | 6.10 | `#f08000` | 5.17 | 6.77 |
| lime | 80° | `#496e00` | 5.10 | 5.98 | `#74ad00` | 5.12 | 6.70 |
| green | 145° | `#007330` | 5.12 | 6.01 | `#00b54b` | 5.12 | 6.70 |
| teal | 175° | `#006e65` | 5.24 | 6.14 | `#00b0a1` | 5.13 | 6.71 |
| cyan | 192° | `#006a85` | 5.27 | 6.18 | `#00abd6` | 5.17 | 6.77 |
| sky | 205° | `#0065ad` | 5.17 | 6.06 | `#24a4ff` | 5.20 | 6.80 |
| blue | 218° | `#0055e8` | 5.17 | 6.06 | `#669eff` | 5.24 | 6.86 |
| indigo | 250° | `#5938ff` | 5.21 | 6.11 | `#a28fff` | 5.25 | 6.87 |
| violet | 272° | `#8700fc` | 5.21 | 6.11 | `#c480ff` | 5.23 | 6.84 |
| magenta | 310° | `#b50097` | 5.25 | 6.16 | `#ff59e3` | 5.17 | 6.77 |
| pink | 335° | `#c40052` | 5.16 | 6.05 | `#ff69a7` | 5.20 | 6.80 |
| grey | 220° | `#5e6268` | 5.23 | 6.14 | `#9b9ea4` | 5.19 | 6.79 |

- 池内最小:浅色 **5.10**(对 #ebedf0)、深色 **5.12**(对 #2c2c2e);对真实底色更高(浅色池对白 ≥5.98,深色池对 base ≥6.70)。**13/13 全部达到正文级 AA**,且留了余量以吸收取整与悬停态。
- 同槽位同色相(如 slot 0 两主题都是红系)⇒ 换主题视觉连续;浅色池每色亮度都低于其深色池同槽位色 ⇒ 两套池确实一深一浅。
- 跨主题表现(如实记录,不作承诺):浅色池的色放到深色侧、深色池的色放到浅色侧都只有 2.2~2.4,明显不可用 —— 这正是**必须靠槽位解析换色**而不是「一个色凑合两边用」的原因。
- 旧的单池 13 色(两底 ≥3.0)在 AA 口径下**整池不合格**(其 4 个复用 swatch 在浅侧只有 3.2~3.7),因此被替换;`SWATCHES` 手动色板**保持不变**(手动选色是用户自己的决定,不在这条承诺范围内)。

### 主题信号与重渲染

- 判据 = **`document.body[data-ds-dark-theme]`**(官方 `packages/client/ui-theme/src/boot-theme.ts` 用 `toggleAttribute` 打的布尔属性),**不读 `prefers-color-scheme`** —— 宿主自己解析 `system`,可能与 OS 相反。
- 模块级 store + `React.useSyncExternalStore`(缺该 API 时退化为直接读)`useAutoThemeDark()` 挂在 BetterBrowser 与 AppearanceControls 上;官方换主题时 body 属性变更触发 MutationObserver → 重渲染 → 行色换成另一套池。渲染期与快照读的是同一个模块级标志,不存在两次渲染读到不同主题的撕裂。

### 文案口径(21 门语言全改)

hint / README(中英)/ 代码注释从「两底 ≥3.0;正文 AA 未达」改为「每个色在**对应主题**背景下 **≥4.5(WCAG 正文级 AA)**;切换主题自动换成同色系另一套」。冒烟守卫同步**反转**:逐门断言 21 份 hint 都含 `4.5`/`4,5` 与 `AA`,且**不得再出现 `3.0`/`3,0` 旧阈值**;另断言 zh 不再出现「未达」、en 不再出现「does not reach」。

### 独立验收发现并修复 P1(v0.25.0,未发版):切主题后行不重绘
...
- **修复后真机验证(隔离 DSH_HOME + 端口 19417 + Playwright,13/13,双向均不刷新)**:新会话拿到 `auto:2`(青柠槽)。
  ...
  - **反向**点回浅色:0.5s 采样即回到 `rgb(73,110,0)`,实测 5.63 ⇒ 双向都自动换池。
  - 两个方向都断言**存储里的 `auto:2` 一字未改**(换主题只重绘、不写数据);console 0 报错。
- **AGENTS**:不变量 15 补"承诺的边界"(第三方背景插件把侧栏底色改得比判据更极端时,AA 承诺不再自动成立;与不变量 14 同源)。注:AGENTS 里有两个编号 10(peer 声明那条与压缩树那条),该句按语义归入**不变量 15**(AA 承诺的所在地)。

### 真机验证(隔离 DSH_HOME + 端口 19416 + Playwright,22/22)

隔离实例装本地 0.25.0 tarball。**实测背景**:浅色 `body` = `rgb(255,255,255)`;深色 `body` = `rgb(21,21,23)`;行底色是把该行半透明叠加合成到底色后的有效值 —— 浅色下当前行 `rgb(247,248,250)`,深色下 `rgb(39,40,43)`(判据用的 #ebedf0 / #2c2c2e 都比它们更保守,实测对比度因此更高)。

- **A 段(13/13,双主题 + 换主题)**:
  - 存储:**分配的样式就是 token** —— `session:session-11f46d4b…` = `{"color":"auto:3"}`,没有任何 hex 写入。
  - 浅色主题:同一行渲染 `rgb(0,115,48)` = `#007330`(slot 3 浅色池绿),**实测对比度 5.66**(对 `rgb(247,248,250)`),≥4.5 ✓。
  - 切成深色主题(官方设置页 通用设置 → 深色,真实点击):**同一行自动变成 `rgb(0,181,75)` = `#00b54b`(同槽位深色池绿),实测对比度 5.39**(对 `rgb(39,40,43)`)✓;同时断言**存储里的 `auto:3` 一字未改**(换主题不写数据)✓。
  - 切回浅色:颜色**还原**为 `rgb(0,115,48)` ✓。
  - 深色主题下新建工作区:拿到 `{"color":"auto:4","icon":"IconSparkle16"}`(token + 图标),行渲染 `rgb(0,176,161)` = `#00b0a1`,实测 5.40 ✓。
  - console 0 报错。
- **B 段(9/9,既有红线回归)**:
  - 开关打开后刷新:首帧已存在的 blank 行**没有被染色**(播种逻辑在新代码里依旧生效)✓。
  - 新建会话被分配 `auto:4` → 右键「自定义外观」手动改成**不在任何池内**的 hex `#3fb950` → 刷新后仍是 `#3fb950`(存在性门控挡住覆盖)✓。
  - 关闭开关后新建项**零分配**,已分配的样式**不回收** ✓。
- 截图:`shot-aa-light.png`(浅色,绿字行)/ `shot-aa-dark-ws.png`(深色,新工作区青绿 + 图标、会话浅蓝)。收尾已 kill 实例、删隔离目录、确认端口关闭。

### 未验证 / 已知限制

- **桌面端(Electron renderer)未跑**:开关只读 `prefs`、只写 `styling`,主题信号读的是宿主打的 `body[data-ds-dark-theme]`(桌面与 web 同一套 shell 资产),但按仓库纪律如实标注。
- **行底色判据的边界**:判据取「浅色悬停面 #ebedf0 / 深色最亮面 #2c2c2e」两个保守值。若用户用第三方背景插件(dsh-any-background)把侧栏改得比这更亮/更暗,对比度承诺不再自动成立 —— 这与既有「透明/磨砂适配」纪律同源(自绘面跟随主题 token,但无法预知任意背景图)。
- **槽位表 append-only**:未来若删除/重排槽位,旧设备的旧快照可能只存在于宿主副本里 ⇒ 已明确「越界夹到 slot 0」并写进 AGENTS 不变量 15。

### 测试

`npm test` **55/55**。调色板测试整体重写为双主题 AA 断言:槽位数 12~16、token 唯一、浅色池逐色对 `#ebedf0` **且**对 `#ffffff` ≥4.5、深色池逐色对 `#2c2c2e` **且**对 `#151517` ≥4.5、浅色池亮度一律低于同槽位深色池、token 解析(两个主题各自解析、越界夹到 slot 0、hex 原样透传、空值透传);洗牌袋测试改为按 **token** 抽取(主题切换不影响「最近抽过」)。
...
- **残余理论窗口(非缺陷,后续项):宿主分批推送会话列表时,"页面加载前就已存在"的 blank 行若晚到,仍可能被染色**。当前判据用「首份**有内容的**权威快照」播种,能覆盖三种情形:①快照为空(不消耗标志,等下一份);②快照只含已命名会话(没有 blank 行可登记 ⇒ 不置位,下一份再试);③快照含 blank 行(登记、不染色)。**覆盖不了的是「同一批存量被拆成多次推送、且首次推送里没有 blank 行」**——此时晚到的那个 blank 行在判据上就等同于"新会话"。要彻底关闭这个窗口,需要宿主提供"会话列表已完整/已收敛"的信号(工作区侧已有先例:`workspaceStreamState !== 'loading'`;会话侧目前只有 `phase: 'pending' | 'ready'`,没有等价的收敛态)。届时把播种条件从"有内容"改成"已收敛"即可,客户端逻辑不用重写。**影响评估**:需要宿主在极小的时间窗内分批推送、且用户恰好在首帧就有 blank 占位行、且开关已开;命中的后果只是"给一个从未用过的占位会话染了色",不会覆盖任何手动外观,也不影响已有会话。
- **验收置信度说明(如实记录)**:1/8 的偶发率下,3 轮冷启动全清的概率只有约 **67%**(0.875³ ≈ 0.67),所以**结论主要来自代码判据的结构性论证**(原先那个"空快照消耗播种标志"的窗口在判据层面已不成立:空快照不再置位),3 轮真机是**辅助证据**,不是"数学证明归零"。后续如需更强证据,应把宿主分批推送的场景做进夹具或加收敛信号。
