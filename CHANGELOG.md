...
**颜色作用面(决定了调色板怎么挑)**:本插件的行级外观里 `color` 落在 `rowStyleOf()` 的 `style.color`,也就是**整行文字色**(外加可选 textShadow/描边),不是装饰色点。因此调色板按**文字可读性**定标:每个色在 `#ffffff`(浅色侧栏)与 `#1c1c1e`(深色侧栏)上的 WCAG 对比度**都 ≥ 3.0**。
...
- **新会话** = wire 仍报 `blank === true`(仓库既有认定的唯一可信出生标记,quote-on-land 同款判据)。存量会话永不 blank ⇒ 刷新页面绝不会把既有会话重新染色。
...
⚠️ **行为边界(如实记录)**:blank 占位会话(「新会话」行,从未发过消息)在**每次页面加载**时都算"新会话"⇒ 若开关打开后刷新页面,这个占位行也会被分到颜色。存量**有内容**的会话永不 blank,因此不会被染色或改色;真机 C 段正是用这一点验证了"已有 entry 优先"。
