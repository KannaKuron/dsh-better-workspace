...
![工作区分层树预览](docs/screenshots/1-workspace-tree.png)

![外观自定义效果预览(颜色/发光/字体粗细/阴影/图标)](docs/screenshots/2-customized-tree.png)

![自定义外观弹窗(实时预览)](docs/screenshots/3-appearance-dialog.png)

![插件设置卡片(设置 → 插件 → 插件配置)](docs/screenshots/4-settings-card.png)

![右键菜单(操作都收进右键:重命名/删除/自定义外观)](docs/screenshots/5-context-menu.png)
...
- **单链分组合并显示(VS Code 式)**:当分组链上每层只有一个子级(如 `测试层1` 下只有 `AI交易` 一个工作区)时,合并成一行显示 `测试层1/AI交易`;某层出现多个子级时自动恢复为树状。可在设置页开关。
- **外观自定义**:右键任意一层(工作区分组 / 工作区 / 会话 / 会话分组)弹出菜单:重命名/删除/分叉/归档等操作 +「自定义外观」——颜色(预设色板 + 取色器 + RGB)、发光强度(0..14 拖动条)、字体粗细(常规/中/半粗/粗)、字体阴影开关、图标(与 dsh 图标库同族的 66 个,工作区与工作区分组专属)、底部实时预览。清除自定义一键复原。
- **设置卡片**:设置 → 插件 → 插件配置 里新增「更好的工作区」卡片(官方折叠卡片样式),提供单链合并开关;展开状态与外观自定义持久化在当前浏览器(dsh 客户端 store)。
- **分组级操作**:分组行右键——重命名分组(同步更新组内所有工作区名称)、删除空分组。
