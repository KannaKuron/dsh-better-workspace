...
- **新增项自动配色 + 图标(v0.24.0,默认关闭)**:设置卡里一个开关,打开后**新创建的会话**自动分到一个内置色、**新工作区**再额外分到一个图标,懒得逐个设置的人直接开箱即用。颜色来自**定死的 13 色调色板**,按「洗牌袋 + 近期回避窗口」抽取——一轮之内绝不重复,跨轮也不会立刻撞上刚出过的 3 个,连续新建几项肉眼可辨;每个色在浅色与深色主题上都保证文字可读(两底对比度 ≥ 3.0,测试里直接锁死)。**只影响打开开关之后新建的项**:你手动设置过外观的行永不被覆盖,关闭开关也不会回收已分配的样式,同一项不会被二次随机。图标从既有图标候选集里取、按宿主实际导出过滤;**图标只给工作区**——会话行的那个位置是官方状态灯与日程角标,没有图标位。
