...
- **外观自定义**:右键任意一层(工作区分组 / 工作区 / 会话 / 会话分组)弹出菜单:重命名/删除/分叉/归档等操作 +「自定义外观」——颜色(预设色板 + 取色器)、发光强度(无/弱/中/强)、文件夹图标样式(实心/空心/不显示,与 dsh 图标库同族)。清除自定义一键复原。
- **设置页**:官方设置面板新增「更好的工作区」标签(`settings.section`),目前提供单链合并开关;展开状态与外观自定义持久化在当前浏览器(dsh 客户端 store)。
