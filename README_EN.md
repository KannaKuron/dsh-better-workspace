...
- **Auto color + icon for new items (v0.24.0, off by default)**: one switch in the plugin settings card. Turn it on and every newly created session gets a built-in color while every new workspace additionally gets an icon — for anyone who would rather not style rows one by one. Colors come from a **fixed 13-color palette** drawn with a shuffle bag plus a recent-draw window: nothing repeats inside a cycle, and a refill never starts with the last three colors, so items added in a row look clearly different. On readability, stated exactly: every entry keeps contrast **≥ 3.0** on both the light and the dark background (the WCAG graphics / large-text threshold); **normal-text AA asks for 4.5 and this pool does not reach it** — the same tier as the plugin's existing manual swatches, and the five weakest of those (2.0–2.8 on white) were excluded. A single fixed hex cannot satisfy pure white and near black at once; that is a mathematical constraint, not a missed tradeoff. **Only items created after you turn it on are touched**: rows you styled by hand are never overwritten, turning it off does not undo assignments, and an item is never randomized twice. Icons come from the existing icon candidate set, filtered by what the host actually exports — and **workspaces only**: the session row's cell belongs to the official status dot and schedule mark, so it has no icon slot.
